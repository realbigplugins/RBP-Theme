...
add_action( 'add_meta_boxes', function() {
    ...
    add_meta_box(
        THEME_ID . '_testimonials_meta_box', // Metabox ID
        sprintf( __( '%1$s Testimonials', 'easy-digital-downloads' ), edd_get_label_singular(), edd_get_label_plural() ), // Metabox Label
        'rbp_download_testimonial_meta_box', // Callback function to populate Meta Box
        'download',
        'normal', // Position
        'high' // Priority
    );
    ...
    add_meta_box(
        THEME_ID . '_features_meta_box', // Metabox ID
        sprintf( __( '%1$s Features', 'easy-digital-downloads' ), edd_get_label_singular(), edd_get_label_plural() ), // Metabox Label
        'rbp_download_features_meta_box', // Callback function to populate Meta Box
        'download',
        'normal', // Position
        'high' // Priority
    );
    ...
    add_meta_box(
        THEME_ID . '_video_meta_box', // Metabox ID
        sprintf( __( '%1$s Video', 'easy-digital-downloads' ), edd_get_label_singular(), edd_get_label_plural() ), // Metabox Label
        'rbp_download_video_meta_box', // Callback function to populate Meta Box
        'download',
        'normal', // Position
        'high' // Priority
    );
 ...
 * @return void
 */
function rbp_download_testimonial_meta_box() {
    
    rbm_do_field_repeater( 'testimonials', false, array(
        'gravatar_email' => array(
            'type' => 'text',
            'label' => __( 'Gravatar E-mail', THEME_ID ),
        ),
        'content' => array( 
            'type' => 'wysiwyg',
            'label' => __( 'Content', THEME_ID ),
            'wysiwyg_args' => array(
                'tinymce' => true,
                'quicktags' => true,
            ),
 ...
 * @return void
 */
function rbp_download_features_meta_box() {
 ...
 * @return void
 */
function rbp_download_video_meta_box() {
 ...
 * @return void
 */
function rbp_download_requires_meta_box() {
    
    rbm_do_field_repeater( 'requirements', false, array(
        'requirement' => array(
            'type' => 'text',
            'label' => __( 'Requirement', THEME_ID ),
        ),
    ) );
